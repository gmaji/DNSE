...
/**
 * Code to get centrality measure values for each node for all methods. This can be plotted pair-wise to get the correlation 
 * or may be plotted against SIR spreadability in a pair-wise manner.
 */
import java.io.File;
...
public class Centrality_Score_Correlation {
...
//				{"Test",	    "test.txt",	        "0.31",		"0.4",		    "0.35"},// V:13;  E:18
				{"Toy_DNS",		"toy_dns.txt",	    "0.276",	 "0.4",		    "0.30"}, // V:20, E: 					    
								};
		...
		for (int i=0; i < networks.length; i++) {
			...
			PrintStream out = new PrintStream(new File("results/dns/corr."+networks[i][1]));
			...
			out.printf("KS+    \t");
			out.printf("KSW    \t");
			out.printf("KSW_GM  \t");//same as KS_PEW in paper
			out.printf("MDD \t");
			out.printf("THETA \t");
			out.printf("LSS \t");
			out.printf("DNSE  \t");
			...
			for (String v: graph.getVertices()) {
				out.printf("%s\t", graph.getVertex(graph.getIndex(v)));
				out.printf("%1.3f\t", epidemicRanker.getNodeValueDouble(v));
				out.printf("%d\t", degreeRanker.getNodeValue(v));
				out.printf("%d\t", kshellRanker.getNodeValue(v));
				out.printf("%d\t", kshellplusRanker.getNodeValue(v));
				out.printf("%d\t", kshellW.getNodeValue(v));
				out.printf("%1.3f\t", ksW_GM.getNodeValueDouble(v));
				out.printf("%1.3f\t", mddRanker.getNodeValueDouble(v));
				out.printf("%d\t", improvedRanker.getNodeValue(v));
				out.printf("%d\t", lss.getNodeValue(v));
				out.printf("%1.3f\t", dns2Ranker.getNodeValueDouble(v));
